...
def main():
    ...
    for _ in range(t):
        ...
        for i in range(n):
            ...
            # print(val, s)
            if val not in s:
                output_list += [' '.join(map(str, [t1[i], t2[i]]))]
                f=True
                break
            elif 2*t1[i] not in s:
                output_list += [' '.join(map(str, [t1[i], t1[i]]))]
                f=True
                break
            elif 2*t2[i] not in s:
                output_list += [' '.join(map(str, [t2[i], t2[i]]))]
                f=True
                break
        if not f:
            output_list += [-1]
